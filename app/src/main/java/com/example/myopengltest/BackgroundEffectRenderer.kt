...
class BackgroundEffectRenderer : GLSurfaceView.Renderer {
	...
	private val fragmentShaderCode = """
		...
		varying vec2 v_UV;  // (0,0)~(1,1)
		uniform float u_Time;
		
		void main() {
			float wave = 0.5 + 0.5 * sin(u_Time + v_UV.x * 10.0);
			gl_FragColor = vec4(wave, v_UV.y, 0.5, 1.0);
		}
    """.trimIndent()
	// endregion
