...
class RadialEffectRenderer : GLSurfaceView.Renderer {
	...
	private val fragmentShaderCode = """
		...
		void main() {
			vec2 center = vec2(0.5, 0.5);                // 중심점
			float dist = distance(v_UV, center);         // 거리 계산 (0.0 ~ sqrt(0.5))
		
			// 반경을 시간에 따라 애니메이션
			float radius = 0.3 + 0.1 * sin(u_Time);      // 0.2~0.4 사이 진동
